...
describe('BookComponent', () => {
  ...
  it("Must render two buttons with values Delete and Edit", () => {
    ...
    for(let i = 0; i < buttons.length; i++) {
      expect(buttons[i].textContent).withContext("Buttons must be created.").toEqual(['Edit', 'Delete'][i]);
    }
  });
  ...
  it("Book must be rendered with red colored button.", async () => {
    ...
    expect(style.backgroundColor).withContext('Color must be red unhovered.').toEqual('rgb(138, 2, 2)');
  })
});
